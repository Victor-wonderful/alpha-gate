...
★ 강한 추세 진입 방식 — "즉시 진입" 우선 (★ 라이브 데이터 검증, 매우 중요):
- 강한 추세(strength="strong")에선 1차 진입(entries[0])을 **현재가(ticker.last) 근처 즉시**로 잡아라. entryType="immediate".
- 이유: 라이브 데이터상 "되돌림 대기" 지정가 진입은 89%가 체결조차 안 됨(가격이 안 돌아옴) → 추세 기회를 통째로 놓침. 강한 추세는 깊은 눌림이 잘 안 온다.
- 1차는 현재가 ±(스타일 immediate 한도의 절반 이내), 2차/3차만 얕은 눌림 add로. 돌파 추격(현재가 위 stop 진입)도 허용.
- 절대 강한 추세에서 1차 진입을 현재가에서 먼 깊은 되돌림(체결 안 될 자리)에 두지 마라.

★ 레짐별 셋업·확신 안내 — "항상 거래 가능, 단 정직하게" (★ 매우 중요):
어떤 시장 상황이든 **최소 1개 이상 거래 셋업을 반드시 제시하라. scenarios를 빈 배열로 두지 마라.** 단 확신을 솔직히 표시:
- 강한 추세(strong): 추세 방향 추종(trend_pullback/breakout). 확신 높음. warnings에 따로 경고 불필요.
- 중간 추세(moderate): 추세 방향. 확신 중간 — warnings에 "중간 추세 — 사이즈 절반 권장" 명시.
- 약한 추세/횡보/혼조(weak·range·mixed): **박스 돌파(breakout)로 새 추세를 노려라**(페이드 금지 — 손실 검증). 진입은 박스 끝 돌파 확정 시(돌파 추격 stop). 확신 중간 — warnings에 "약세/횡보 — 돌파 대기, 사이즈 축소" 명시.
...
- 손절폭은 진입가의 **최소 0.225%** (수수료 왕복 0.075% × 3) 이상이어야 한다.
- 손절폭이 좁을수록 손절 적중 시 수수료가 손절(1R) 대비 큰 비중을 차지한다. 0.225%에서도 수수료 포함 약 1.3R 손실이며, 그보다 좁으면 더 커지므로 0.225%가 절대 하한이다.
- 이는 모든 스타일·모든 전략에 적용된다. liquidity_grab·session_open_drive 같은 예외 전략도 0.225% 절대 하한은 못 넘는다.
- 만약 구조상 자연 손절이 0.225% 안에 있다면, 그 시나리오는 만들지 마라 (wait 또는 다른 셋업 찾기).

스타일별 손절/목표 표준 (위 절대 하한 위에 추가로 적용 — 테이커+메이커 합쳐 최대 0.075% + 슬리피지 별도):
...
- liquidity_grab: 손절폭이 스타일 하한 미만이어도 허용 (sweep 직전 wickExtreme이 자연 손절). 단 0.225% 미만은 절대 금지. R:R ≥ 2.5 필수. qualityIssues에 "특수 셋업: 손절 좁음 — 리스크% 평소 절반 권장" 추가.
- funding_squeeze: 스타일 범위 그대로 적용. R:R ≥ 2 필수. warning에 "시간 한도 12~24h" 필수.
- session_open_drive: 데이 표준의 절반 손절폭까지 허용 (개장 캔들 시가가 자연 손절). 단 0.225% 미만은 절대 금지. qualityIssues에 "개장 드라이브 — 미국 마감 전 청산" 추가.
...
function enforceEntryProximity(
  ...
  if (isRangeRegime && kept.length > 0) {
    warnings.unshift(
      "현재 횡보·추세 불명확 — 박스 끝 페이드는 손실 검증. 박스 돌파(새 추세)를 노리고, 확신 낮으니 사이즈 축소.",
    );
  }
