...
import { scanKimchi } from "@/lib/arbitrage/scan";
...
export default async function ArbitragePage() {
  ...
  const [wallet, kimchi, openRes, closedRes] = await Promise.all([
    getOrCreateWallet(user.id).catch(() => null),
    scanKimchi(),
    supabase
      .from("arbitrage_positions")
      .select(
        "id, kind, symbol, notional_usd, long_exchange, long_entry_price, long_qty, short_exchange, short_entry_price, short_qty, entry_premium_pct, expires_at, created_at",
      )
      .eq("user_id", user.id)
      .eq("kind", "kimchi")
      .eq("status", "open")
    ...
    supabase
      .from("arbitrage_positions")
      .select(
        "id, kind, symbol, notional_usd, long_exchange, short_exchange, entry_premium_pct, long_entry_price, short_entry_price, long_exit_price, short_exit_price, realized_pnl, close_reason, created_at, closed_at",
      )
      .eq("user_id", user.id)
      .eq("kind", "kimchi")
  ...
  return (
    <div className="space-y-5">
      <ClusterTabs
        title={cluster.title}
        description="시장 간 가격 차이(김프)를 활용한 무방향 차익거래. 양쪽 다리를 동시에 진입/청산합니다."
        tabs={cluster.tabs}
