...
사용자가 차트 이미지를 함께 제공한 경우:
- 스냅샷의 숫자 데이터를 **ground truth**로 취급한다. 이미지에서 읽은 가격이 스냅샷과 다르면 스냅샷을 신뢰한다.
- 이미지는 보조 컨텍스트로만 사용한다: 사용자가 차트에 그린 추세선·박스·노트, 우리가 fetch하지 않은 인디케이터(예: Bollinger Bands, Custom Pine), 우리가 fetch하지 않은 시간프레임의 시각적 구조.
- 이미지에서 의미 있는 추가 정보가 보이면 keyLevels나 scenarios.note에 짧게 반영한다. (예: "사용자 차트에 그려진 추세선이 $43,500에 위치")
- 이미지가 흐릿하거나 읽을 수 없으면 무시하고 데이터만으로 분석한다.`;
...
- 방향이 명시됐다면 그 방향의 시나리오만 만든다. (long이면 long만, short이면 short만)`;
  ...
  if (image) {
    ...
    userBlocks.push({
      type: "text",
      text: "사용자가 함께 첨부한 차트 이미지입니다. 보조 컨텍스트로만 사용하세요.",
    });
  }
