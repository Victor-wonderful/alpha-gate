# Alpha Gate — Project Context for Claude Code
...
| AI | **`@anthropic-ai/sdk`**, model `claude-sonnet-4-6`, **프롬프트 캐싱 필수** |
...
- 왕복 비용 가정: **0.075%** (테이커+메이커 합쳐 최대 — 한쪽 메이커 체결 기준). 슬리피지는 체결가(entry_actual/exit_actual)에서 별도 처리 — 이중차감 방지. 절대 손절 하한 = 0.075%×3 ≈ **0.225%**.
...
8. **언어**: 전문 용어 직접 사용 → **평범한 한국어 강제** (Claude 프롬프트로)
...
4. **Claude JSON 응답 잘림** — max_tokens 3000으로 상향 + 응답 길이 제한 명시. [`json-extract.ts`](src/lib/analysis/json-extract.ts)로 강건한 파싱.
