...
제약:
- 시나리오는 1~3개. entryZone, invalidation, target은 모두 숫자. 누락 불가.
- range_fade 전략에서는 양방향 시나리오 만들기 가능 (박스 상단 숏 + 박스 하단 롱).
- trend_pullback 전략에서는 추세 방향 시나리오만 (반대 방향 금지).
...
★ 특수 전략 손절폭 예외 (위 표준보다 우선):
- liquidity_grab: 손절폭이 표준 하한 미만이어도 허용 (sweep 직전 wickExtreme이 자연 손절). 단, R:R ≥ 2.5 필수. qualityIssues에 "특수 셋업: 손절 좁음 — 리스크% 평소 절반 권장" 추가.
- funding_squeeze: 표준 범위 그대로 적용. R:R ≥ 2 필수. warning에 "시간 한도 12~24h" 필수.
- session_open_drive: 데이 표준의 절반 손절폭까지 허용 (개장 캔들 시가가 자연 손절). qualityIssues에 "개장 드라이브 — 미국 마감 전 청산" 추가.
